    ...
    if (choice === "accept") {
      if (p.kind === "PITCHER") {
        ...
        log({
          ...
          body: toCp
            ? "선발 로테이션을 떠나 뒷문 쪽으로 갑니다. 한 이닝이면 아직 누구보다 셉니다."
  ...
  prompt: (s) => {
    ...
    return toSP
      ...
      : {
          key: "ROLE_SWITCH", icon: "🔒",
          title: "마무리 전환 제안",
          body: "짧게 윽박지르는 구위입니다. 구단이 뒷문 쪽에서 키우려 합니다.",
          options: [
            { id: "accept", label: "뒷문 쪽으로 간다", desc: "9회를 맡으면 세이브가 쌓이고 이름이 알려집니다. 다만 그 자리는 기량으로 따내야 합니다." },
            { id: "refuse", label: "지금 자리를 지킨다", desc: "승부처는 앞에서도 옵니다. 부담이 덜한 대신 눈에 덜 띕니다." },
          ],
        };
  },
  /**
   * 자리를 옮기는 것과 **9회를 맡는 것은 다르다.**
   * 보직은 이듬해 캠프에서 기량으로 다시 정해지므로(`assignRole`), 전환 통보가
   * "9회를 맡습니다"라고 못 박으면 정작 필승조로 시작하는 해가 생긴다.
   * (실제로 겪음: 서른일곱에 "마무리 전환"이 뜨고 그해 보직은 필승조였다)
   */
  apply: (s, choice, rng, log) => {
    const p = s.player;
    if (choice !== "accept") {
    ...
    log({
      ...
      body: toSP
        ...
        : "뒷문 쪽으로 자리를 옮깁니다. 9회는 기량이 되는 만큼 돌아옵니다.",
    });
  },
  resolve: (s, choice, rng, log) => {
    if (choice !== "accept") return;
    ...
    log({
      icon: sp ? "🔄" : "🔒",
      title: sp ? "선발이 몸에 붙었다" : "짧은 이닝이 몸에 붙었다",
      tone: "good",
      body: sp
        ? "한 해를 로테이션에서 보내며 이닝을 버티는 몸이 됐습니다."
        : "짧게 끊어 던지는 일에 익숙해졌습니다. 마지막 아웃의 무게를 압니다.",
    });
  },
};
