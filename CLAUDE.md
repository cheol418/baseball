...
`titles`(성적 대비 수상), `titlebar`(부문별 기준선·수상률), `nego`(협상 도박 균형), `traingrade`(훈련 성과 분포), `randomcreate`(랜덤 조합 정합성), `agelimit`(AG 연령 제한), `asscore`(올스타 스코어↔승패), `asorder`(올스타 정보 유출), `textcheck`(문구 빈칸),
